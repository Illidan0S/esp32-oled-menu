/*
* 28-07-2026  : 22:30 -> 01:00
* versione 2.5
* Versione precedente: 2.4
* Aggiornamenti:
...
unsigned long tempoFacce = 0;
unsigned long tempoUltimoAgg = 0;
const char* lista[] = {
  "Bro",
  "AURA",
  "Canzoni",
...
void impostaEspressione(){
  switch (Espressioni[faccia]){
    case NORMALE:
      roboEyes.open();
      ...
      roboEyes.setAutoblinker(true, 2, 5);
      break;
    ...
    case TRISTE:
      ...
      roboEyes.setHeight(36, 36);
      if (faccia % 2 == 0){
        roboEyes.setPosition(S);  // guarda sotto
      }
    ...
    //roboEyes.setPosition(NE);   // guarda in alto a destra
    if (faccia%2 == 1){
      roboEyes.setPosition(E); // guarda a destra
      //tempo-=500;
    } else{
      roboEyes.setPosition(W);  // guarda a sinistra
      //tempo-=500;
    }
    break;
...
void loop() {
  ...
  if (schermata == 0 && LightSleepMode == false){
    ...
    if (statoAttD == false && millis()-tempo>200){
  ...
  } else if (schermata == 1 && LightSleepMode == false){
      ...
      audio.loop();
    statoPrecB = statoAttB;
  } else if (schermata == 2 && LightSleepMode == false){
      ...
      audio.loop();
      statoPrecB = statoAttB;
  } else if (schermata == 3 && LightSleepMode == false){
    if (st_sch3 == false){
      st_sch3 = true;
      for (int f = 0; f < 6; f++) {
  ...
  //Gestione animazione LightSleepMode
  if (LightSleepMode == false && millis()-tempo>45000){
    LightSleepMode=true;
    Serial.println(F("Sleep mode attivata!"));
    //stoppa eventuale audio in riproduzione
    audio.stopSong();
    // Disattiva i comportamenti precedenti di BRO
  ...
  if (!AnimazioneSleep && LightSleepMode==true){
    roboEyes.update();
    tempoUltimoAgg=millis();
  } else if (millis()-tempoUltimoAgg<500){
      roboEyes.update();
  }
  
// Dopo 45 secondi di inattività mostra l'animazione, poi entra in Light Sleep.
if (LightSleepMode==true){
    if (millis()-tempoFacce>1200){

      tempoFacce=millis();
      if (!AnimazioneSleep){
        
        impostaEspressione();
        Serial.println("Faccia mostrata!");
        faccia++;
        if (faccia>=8){
          Serial.println("Facce finite!");
          faccia=0;
          AnimazioneSleep = true;       
        }
      } else{
          // Disegna le Z quando l'animazione è terminata
          display.setTextColor(SH110X_WHITE);
          display.setTextSize(1);
